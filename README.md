📌 PDHG Poisson-TV (Numba Optimized)

Ultra-optimized implementation of the Primal-Dual Hybrid Gradient (PDHG) algorithm for image reconstruction under Poisson noise with isotropic Total Variation (TV) regularization.

⚡ Features
🚀 Numba JIT acceleration with parallel execution (prange)
📉 Poisson noise model (log-likelihood proximal)
🧼 Isotropic Total Variation (TV) regularization
🔁 Chambolle–Pock (PDHG) optimization scheme
🧠 Low-level optimizations (branch reduction, cache-friendly loops)
